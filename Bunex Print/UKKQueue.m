            ...
            NS_DURING
                ...
                if( n > 0 )
                {
                    if( ev.filter == EVFILT_VNODE )
                    {
                        if( ev.fflags )
                        {
                            NSString*		fpath = (__bridge NSString *)ev.udata;    // In case one of the notified folks removes the path.
...
-(void) postNotification: (NSString*)nm forFile: (NSString*)fp
{
	if( delegateProxy )
    {
        ...
        {
            NSLog(@"send folder event to delegate");
